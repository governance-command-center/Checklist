...
function updateUserProgress() {
  ...
  // Legacy progress bar (hidden, but keep DOM consistent)
...
function switchAdminTab(tab) {
  const tabs = ['dashboard', 'calendar', 'data', 'registration', 'alerts'];
  tabs.forEach(t => {
...
function injectBrandSummaryRow() {
  // In the new layout the brand-summary-row div already exists in the HTML dashboard
...
// Helper refs
const POLL_META_REF = () => db.collection('settings').doc('activePoll');
const POLL_RESP_REF = () => db.collection('settings').doc('pollResponses');
...
async function createRegPoll() {
  ...
  try {
    ...
    // Save poll metadata to settings/activePoll (replaces any previous poll)
    await POLL_META_REF().set({
      id:        pollId,
      name, desc,
      status:    'open',
...
async function loadAndRenderRegPollAdmin() {
  ...
  try {
    // Read poll metadata from settings/activePoll (permitted collection)
    const pollDoc = await POLL_META_REF().get();
    const poll = pollDoc.exists ? { id: pollDoc.data().id || 'activePoll', ...pollDoc.data() } : null;
    ...
    // Load responses from settings/pollResponses (flat map { uid: responseData })
    const respDoc = await POLL_RESP_REF().get();
...
async function closeRegPoll() {
  ...
  await POLL_META_REF().update({ status: 'closed' });
...
async function checkForPendingPoll() {
  ...
  try {
    // Read from settings/activePoll (permitted collection)
    const pollDoc = await POLL_META_REF().get();
    if (!pollDoc.exists) return;
    const openPoll = { id: pollDoc.data().id || 'activePoll', ...pollDoc.data() };
    if (openPoll.status !== 'open') return;

    const pollId = openPoll.id;

    // Check if user has already responded (settings/pollResponses flat map)
    const respDoc = await POLL_RESP_REF().get();
    const allResponses = respDoc.exists ? respDoc.data() : {};
...
async function submitUserRegistration() {
  ...
  try {
    // Write to settings/pollResponses flat map using dot-notation field update
    await POLL_RESP_REF().set({
      [currentUser.uid]: {
        status:        'registered',
