...
async function loadAndRenderRegPollAdmin() {
  ...
  try {
    // Load all polls then find the open one (avoids composite index requirement)
    const allSnap = await db.collection('registrationPolls').get();
    let allPolls = [];
    allSnap.forEach(d => allPolls.push({ id: d.id, ...d.data() }));
    // Sort by createdAt descending (handle Timestamp or string)
    allPolls.sort((a, b) => {
    ...
    const latestPoll = openPoll || allPolls[0];

    // Fake a snap-like interface
    const fakeDoc = latestPoll ? { id: latestPoll.id, data: () => latestPoll } : null;
    const snap = { empty: !fakeDoc, docs: fakeDoc ? [fakeDoc] : [] };
      ...
      host.innerHTML = `
        <div style="padding:3rem;text-align:center;">
          ...
          <div style="color:var(--text-muted);margin-bottom:24px;max-width:400px;margin-left:auto;margin-right:auto;line-height:1.6;">
            Create a poll to collect brand, platform and region registrations from your team. A "Register Now" broadcast is sent automatically.
          </div>
    ...
    <div class="reg-poll-admin-wrap">
      ...
      <div class="reg-poll-header-card">
        ...
        <div class="reg-poll-header-right">
          <button class="btn-outline" style="font-size:12px;" onclick="openRegPollModal()">📣 New Poll</button>
