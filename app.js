...
function showCatStatusPicker(anchorCb, catId, entryIdx, tab) {
  ...
  picker.style.cssText = `
    position:fixed;z-index:9999;background:var(--navy);border:1px solid rgba(255,255,255,0.2);
    border-radius:8px;padding:6px;display:flex;flex-direction:column;gap:4px;
    box-shadow:0 8px 24px rgba(0,0,0,0.35);min-width:130px;
  `;
  ...
  statuses.forEach(s => {
    ...
    btn.onclick = () => {
      ...
      picker.remove();
      anchorCb.checked = false; // reset the checkbox after applying
    };
      ...
      catEntryCells += `
        <td style="padding:4px 8px;text-align:center;${borderL}">
          <input type="checkbox" class="cat-d5-cb" data-cat-id="${sec.id}" data-entry-idx="${ei}"
            onchange="toggleCatD5(this,'${sec.id}',${ei})" title="Click to bulk-set D-5 status"
        ...
        <td style="padding:4px 8px;text-align:center;">
          <input type="checkbox" class="cat-d1-cb" data-cat-id="${sec.id}" data-entry-idx="${ei}"
            onchange="toggleCatD1(this,'${sec.id}',${ei})" title="Click to bulk-set D-1 status"
