...
function switchAdminTab(tab) {
  ...
  const regTabEl = document.getElementById('admin-tab-registration');
  if (regTabEl) regTabEl.style.display = tab === 'registration' ? 'block' : 'none';
  document.querySelectorAll('.admin-tab-btn').forEach(b => b.classList.remove('active'));
  const btn = document.getElementById('atab-' + tab);
  if (btn) btn.classList.add('active');

  // Show/hide the header Poll shortcut button
  const pollHeaderBtn = document.getElementById('admin-header-poll-btn');
  if (pollHeaderBtn) pollHeaderBtn.style.display = tab === 'registration' ? 'inline-flex' : 'none';
...
async function closeBroadcastFeedAndOpenPoll(pollId) {
  ...
  try {
    ...
    // Load poll data
    const pollDoc = await db.collection('registrationPolls').doc(pollId).get();
    ...
    currentPollData   = { id: pollId, ...pollDoc.data() };
    openUserRegModal();
  } catch(e) {
...
async function createRegPoll() {
  ...
  try {
    // Close any existing open polls (fetch all, filter in JS to avoid index issues)
    const existingSnap = await db.collection('registrationPolls').get();
    const closeBatch = db.batch();
    existingSnap.forEach(d => {
      if (d.data().status === 'open') closeBatch.update(d.ref, { status: 'closed' });
    });
...
async function loadAndRenderRegPollAdmin() {
  ...
  try {
    // Fetch all polls (no composite index needed), sort + filter in JS
    const allSnap = await db.collection('registrationPolls').get();
    const allPolls = [];
    allSnap.forEach(d => allPolls.push({ id: d.id, ...d.data() }));
    allPolls.sort((a, b) => {
    ...
    const poll = allPolls.find(p => p.status === 'open') || allPolls[0] || null;
    ...
    // Load responses sub-collection
    const respSnap = await db.collection('pollResponses').doc(poll.id).collection('responses').get();
    pollResponses = {};
    ...
    host.innerHTML = `
      <div style="padding:3rem 2rem;text-align:center;max-width:540px;margin:0 auto;">
        ...
        <div style="color:var(--text-muted);font-size:13px;line-height:1.7;margin-bottom:20px;">
          ${isPermErr
            ? 'Your Firestore security rules need to allow reads on <code>registrationPolls</code> and <code>pollResponses</code>.<br>Add the following to your Firestore rules:<br><br><code style="background:var(--surface2);padding:6px 10px;border-radius:6px;display:inline-block;text-align:left;font-size:12px;">match /registrationPolls/{doc} { allow read, write: if true; }<br>match /pollResponses/{p}/responses/{r} { allow read, write: if true; }</code>'
            : e.message}
        </div>
    ...
    <div class="reg-poll-admin-wrap">
      ...
      <div class="reg-poll-header-card">
        ...
        <div class="reg-poll-header-right">
          <button class="btn-primary" style="font-size:12px;width:auto;padding:7px 16px;background:#4F46E5;" onclick="openRegPollModal()">📋 New Poll</button>
          ${isOpen ? `<button class="btn-outline" style="border-color:#FCA5A5;color:#FCA5A5;font-size:12px;" onclick="closeRegPoll()">🔒 Close Poll</button>` : ''}
        </div>
      </div>
...
async function checkForPendingPoll() {
  ...
  try {
    // Avoid composite index: fetch all, filter in JS
    const allSnap = await db.collection('registrationPolls').get();
    let allPolls = [];
    allSnap.forEach(d => allPolls.push({ id: d.id, ...d.data() }));
    allPolls.sort((a, b) => {
    ...
    const openPoll = allPolls.find(p => p.status === 'open');
    if (!openPoll) return;
