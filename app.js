      ...
      html += teamLeads.map(tl => {
        ...
        managedUids.forEach(uid => {
          ...
          Object.values(campaigns).forEach(camp => {
            ...
            const cl = (allChecklists[uid] || {})[camp.id] || {};
            const d5 = countDone(cl.d5 || {});
            const d1 = countDone(cl.d1 || {});
            const pct = Math.round(((d5 + d1) / (TOTAL_ITEMS * 2)) * 100);
...
async function renderUserDashboard() {
  ...
  try {
    ...
    const myCampaigns = currentUser.role === 'team_lead'
    ...
    for (const camp of myCampaigns) {
      ...
      // Team leads only see their managed members; regular members see all
      const managedUids = (currentUser.role === 'team_lead') ? (currentUser.managedUids || []) : null;
      const assignedUids = managedUids
        ? allAssigned.filter(uid => managedUids.includes(uid))
        : allAssigned;
...
async function renderTeamLeadView() {
  ...
  campsToShow.forEach(camp => {
    (camp.assignedUids || []).filter(u => managedUids.includes(u)).forEach(uid => {
      ...
      const d5 = Math.round((countDone(cl.d5 || {}) / TOTAL_ITEMS) * 100);
      const d1 = Math.round((countDone(cl.d1 || {}) / TOTAL_ITEMS) * 100);
      const avg = Math.round((d5 + d1) / 2);
  ...
  campsToShow.forEach(camp => {
    ...
    campMembers.forEach(uid => {
      ...
      const overallPct = Math.round((d5Pct + d1Pct) / 2);
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">
          <div>
            <div style="display:flex;justify-content:space-between;font-size:11px;font-weight:600;margin-bottom:5px;">
              ...
              <span style="color:var(--navy);">${d5Done}/${TOTAL_ITEMS} &nbsp;${d5Pct}%</span>
            </div>
            <div style="height:7px;background:var(--border);border-radius:4px;overflow:hidden;">
              <div style="height:100%;width:${d5Pct}%;background:${d5Pct===100?'#16a34a':d5Pct>0?'#f59e0b':'#e5e7eb'};border-radius:4px;transition:width .3s;"></div>
            </div>
          </div>
          <div>
            <div style="display:flex;justify-content:space-between;font-size:11px;font-weight:600;margin-bottom:5px;">
              <span style="color:var(--text-muted);text-transform:uppercase;letter-spacing:.04em;">D-1 Checklist</span>
              <span style="color:var(--navy);">${d1Done}/${TOTAL_ITEMS} &nbsp;${d1Pct}%</span>
            </div>
            <div style="height:7px;background:var(--border);border-radius:4px;overflow:hidden;">
              <div style="height:100%;width:${d1Pct}%;background:${d1Pct===100?'#16a34a':d1Pct>0?'#f59e0b':'#e5e7eb'};border-radius:4px;transition:width .3s;"></div>
            </div>
          </div>
        </div>
      </div>`;
      ...
      <div style="background:var(--surface);border:1px solid var(--blue);border-radius:10px;padding:16px 18px;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">
          <div>
            ...
            <div style="height:7px;background:var(--border);border-radius:4px;overflow:hidden;">
              <div style="height:100%;width:${d5Pct}%;background:${d5Pct===100?'#16a34a':d5Pct>0?'#f59e0b':'#e5e7eb'};border-radius:4px;"></div>
            </div>
          </div>
          <div>
            <div style="display:flex;justify-content:space-between;font-size:11px;font-weight:600;margin-bottom:5px;">
            ...
            <div style="height:7px;background:var(--border);border-radius:4px;overflow:hidden;">
              <div style="height:100%;width:${d1Pct}%;background:${d1Pct===100?'#16a34a':d1Pct>0?'#f59e0b':'#e5e7eb'};border-radius:4px;"></div>
            </div>
          </div>
        </div>
      </div>`;
