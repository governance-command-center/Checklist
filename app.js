...
function toggleCatD5(cb, catId, entryIdx) {
  const status = cb.checked ? 'done' : '';
  const items = [...document.querySelectorAll(`.row-cb[data-cat="${catId}"][data-tab="d5"][data-entry-idx="${entryIdx}"]`)];
  items.forEach(rowCb => {
    const itemId = rowCb.dataset.itemId;
    const ei = parseInt(rowCb.dataset.entryIdx);
    ensurePath('d5');
    const key = ei === 0 ? itemId : `${itemId}_e${ei}`;
    userChecklist[selectedCampaignId]['d5'][key] = {
      ...(userChecklist[selectedCampaignId]['d5'][key] || {}), status
    };
    const sel = document.getElementById(`sel-d5-${itemId}-${ei}`);
...
function toggleCatD1(cb, catId, entryIdx) {
  const status = cb.checked ? 'done' : '';
  const items = [...document.querySelectorAll(`.row-cb[data-cat="${catId}"][data-tab="d1"][data-entry-idx="${entryIdx}"]`)];
  items.forEach(rowCb => {
    const itemId = rowCb.dataset.itemId;
    const ei = parseInt(rowCb.dataset.entryIdx);
    ensurePath('d1');
    const key = ei === 0 ? itemId : `${itemId}_e${ei}`;
    userChecklist[selectedCampaignId]['d1'][key] = {
      ...(userChecklist[selectedCampaignId]['d1'][key] || {}), status
    };
    const sel = document.getElementById(`sel-d1-${itemId}-${ei}`);
    if (sel) { sel.value = status; sel.className = `status-sel ${statusClass(status)}`; }
  });
  updateUserProgress();
  saveChecklist();
...
function renderUserChecklist() {
  ...
  CHECKLIST_SECTIONS.forEach(sec => {
    ...
    for (let ei = 0; ei < entryCount; ei++) {
      ...
      catEntryCells += `
        <td style="padding:4px 8px;text-align:center;${borderL}">
          <input type="checkbox" class="cat-d5-cb" data-cat-id="${sec.id}" data-entry-idx="${ei}"
            onchange="toggleCatD5(this,'${sec.id}',${ei})" title="Bulk D-5 for category"
            style="width:14px;height:14px;cursor:pointer;accent-color:#3B82F6;" />
        </td>
        <td style="padding:4px 6px;background:rgba(37,99,235,0.04);"></td>
        <td style="padding:4px 8px;text-align:center;">
          <input type="checkbox" class="cat-d1-cb" data-cat-id="${sec.id}" data-entry-idx="${ei}"
            onchange="toggleCatD1(this,'${sec.id}',${ei})" title="Bulk D-1 for category"
            style="width:14px;height:14px;cursor:pointer;accent-color:#7C3AED;" />
        </td>
        <td style="padding:4px 6px;background:rgba(124,58,237,0.04);"></td>
        <td></td>`;
    }
