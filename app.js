...
async function renderAdminView() {
  ...
  document.getElementById('admin-stats').innerHTML = `
    <div class="stat-card"><div class="label">Total assignments</div><div class="value blue">${total}</div>
      <div class="stat-bar-track"><div class="stat-bar-fill" style="width:100%;background:var(--blue);"></div></div></div>
    <div class="stat-card"><div class="label">Fully complete</div><div class="value green">${complete}</div>
...
function renderDeadlinePanel() {
  ...
  if (upcoming.length === 0) {
    el.innerHTML = '<div style="color:var(--text-muted);font-size:13px;padding:8px 0;">No upcoming events. Add events in the Calendar tab.</div>'; return;
  }
  el.innerHTML = upcoming.map(e => {
    const evDate   = new Date(e.date);
    const diffDays = Math.ceil((evDate - now) / 86400000);
    let cls = 'dp-green', txt = evDate.toLocaleDateString('en-GB',{day:'numeric',month:'short'});
    if (diffDays === 0)      { cls = 'dp-red';   txt = 'Today'; }
    else if (diffDays === 1) { cls = 'dp-red';   txt = 'Tomorrow'; }
    else if (diffDays <= 5)  { cls = 'dp-amber'; txt = `In ${diffDays}d`; }
    return `<div class="deadline-row">
      <div class="deadline-name" title="${escHtml(e.title)}">${escHtml(e.title)}</div>
      <span class="deadline-pill ${cls}">${txt}</span>
    </div>`;
  }).join('');
}
...
async function duplicateCampaign(campId) {
  ...
  } catch(e) { alert('Failed to duplicate campaign. Try again.'); console.error(e); }
}
...
function switchAdminTab(tab) {
  const tabs = ['dashboard', 'calendar', 'data', 'registration', 'reports', 'alerts'];
  tabs.forEach(t => {
...
async function loadBroadcastFeed() {
  ...
  try {
    ...
    snap.forEach(doc => {
      ...
      const typeIcon  = { shoutout: '🏆', nudge: '⏰', custom: '📢', registration: '📋' }[b.type] || '📣';
      const typeColor = { shoutout: '#059669', nudge: '#D97706', custom: '#2563EB', registration: '#7C3AED' }[b.type] || '#64748B';
...
async function loadChecklistOverrides(campaignId) {
  try {
    // If a campaignId is provided, check if it has a template assigned
    ...
    // Fall back to global checklist override
...
// Open the templates manager modal
...
function closeTemplatesModal(e) {
  if (e && e.target !== document.getElementById('templates-overlay')) return;
  document.getElementById('templates-overlay').style.display = 'none';
}

function showTemplatesListView() {
  document.getElementById('tmpl-list-view').style.display = 'block';
  document.getElementById('tmpl-editor-view').style.display = 'none';
}

function showTemplateEditorView() {
  document.getElementById('tmpl-list-view').style.display = 'none';
  document.getElementById('tmpl-editor-view').style.display = 'block';
}

function renderTemplatesList() {
  const list = document.getElementById('tmpl-list');
  if (checklistTemplates.length === 0) {
    list.innerHTML = '<div style="color:var(--text-muted);font-size:13px;padding:12px 0;">No templates yet. Create one below.</div>';
    return;
  }
  list.innerHTML = checklistTemplates.map((t, i) => `
    <div class="tmpl-row">
      <div class="tmpl-row-info">
        <div class="tmpl-row-name">${escHtml(t.name)}</div>
        <div class="tmpl-row-meta">${t.sections ? t.sections.reduce((s,sec)=>s+sec.items.length,0) : 0} items across ${t.sections ? t.sections.length : 0} sections</div>
      </div>
      <div class="tmpl-row-actions">
        <button class="btn-ghost-light btn-sm" onclick="editTemplate(${i})">✏️ Edit</button>
        <button class="btn-ghost-light btn-sm" style="color:#DC2626;border-color:#FCA5A5;" onclick="deleteTemplate(${i})">🗑</button>
      </div>
    </div>`).join('');
}
...
function newTemplate() {
  ...
  templateEditorSections = JSON.parse(JSON.stringify(CHECKLIST_SECTIONS)); // default to current checklist
  document.getElementById('tmpl-name-input').value = '';
  populateTmplEditorSectionSel();
  renderTmplEditorSection();
...
function editTemplate(idx) {
  ...
  templateEditorSections = JSON.parse(JSON.stringify(t.sections || []));
...
function populateTmplEditorSectionSel() {
  const sel = document.getElementById('tmpl-section-sel');
  sel.innerHTML = templateEditorSections.map((s, i) => `<option value="${i}">${escHtml(s.title)}</option>`).join('');
}

function renderTmplEditorSection() {
  const idx = parseInt(document.getElementById('tmpl-section-sel').value) || 0;
  const sec = templateEditorSections[idx];
  if (!sec) { document.getElementById('tmpl-items-list').innerHTML = ''; return; }

  document.getElementById('tmpl-section-header').innerHTML = `
    <div style="display:flex;gap:8px;align-items:center;margin-bottom:10px;">
      <input type="text" id="tmpl-section-title" value="${escHtml(sec.title)}"
        oninput="templateEditorSections[${idx}].title=this.value;document.getElementById('tmpl-section-sel').options[${idx}].text=this.value;"
        style="font-size:14px;font-weight:600;color:var(--navy);border:1px solid var(--border);border-radius:6px;padding:5px 10px;flex:1;" />
      <button class="btn-ghost-light" style="color:#DC2626;border-color:#FCA5A5;" onclick="deleteTmplSection(${idx})">🗑 Delete</button>
    </div>`;

  const list = document.getElementById('tmpl-items-list');
  if (sec.items.length === 0) {
    list.innerHTML = '<div style="color:var(--text-muted);font-size:13px;padding:12px 0;">No items yet.</div>';
    return;
  }
  list.innerHTML = sec.items.map((item, ii) => `
    <div class="editor-item-row">
      <div class="editor-item-num">${ii + 1}</div>
      <div class="editor-item-fields">
        <input type="text" class="editor-item-name" placeholder="Item name" value="${escHtml(item.name)}"
          oninput="templateEditorSections[${idx}].items[${ii}].name=this.value" />
        <textarea class="editor-item-guide" placeholder="Guide question" rows="2"
          oninput="templateEditorSections[${idx}].items[${ii}].guide=this.value">${escHtml(item.guide)}</textarea>
      </div>
      <button class="editor-item-del" onclick="deleteTmplItem(${idx},${ii})">✕</button>
    </div>`).join('');
}
...
function addTmplSection() {
  const name = prompt('Section name:');
  if (!name) return;
  templateEditorSections.push({ id: `sec_${Date.now()}`, title: name, items: [] });
  populateTmplEditorSectionSel();
  document.getElementById('tmpl-section-sel').value = templateEditorSections.length - 1;
  renderTmplEditorSection();
}

function deleteTmplSection(idx) {
  if (!confirm(`Delete section "${templateEditorSections[idx].title}" and all its items?`)) return;
  templateEditorSections.splice(idx, 1);
  populateTmplEditorSectionSel();
  document.getElementById('tmpl-section-sel').value = 0;
  renderTmplEditorSection();
}

function deleteTmplItem(secIdx, itemIdx) {
  if (!confirm('Delete this item?')) return;
  templateEditorSections[secIdx].items.splice(itemIdx, 1);
  renderTmplEditorSection();
}
...
async function saveTemplate() {
  ...
  for (const sec of templateEditorSections) {
    if (!sec.title.trim()) { showError(errEl, 'All sections must have a title.'); return; }
    for (const item of sec.items) {
      if (!item.name.trim()) { showError(errEl, `Empty item in "${sec.title}".`); return; }
    }
  }
  errEl.style.display = 'none';
  ...
  if (editingTemplateId) {
    ...
    if (idx >= 0) checklistTemplates[idx] = { ...checklistTemplates[idx], name, sections: templateEditorSections };
  } else {
    checklistTemplates.push({ id: `tmpl_${Date.now()}`, name, sections: templateEditorSections });
  }
  ...
  } catch(e) { showError(errEl, 'Failed to save template.'); }
}
...
async function createCampaignFromPoll() {
  ...
  if (registered.length === 0) {
    alert('No registrations to create a campaign from.');
    return;
  }
  ...
  try {
    ...
    campList.forEach(camp => {
      ...
      (camp.assignedUids || []).forEach(uid => {
        ...
        const lastStr = cl.lastActive
          ? new Date(cl.lastActive).toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'})
          : '—';
        const status = overallPct === 100 ? 'Complete' : overallPct > 0 ? 'In Progress' : 'Not Started';
        const statusCls = overallPct === 100 ? 'badge-done' : overallPct > 0 ? 'badge-partial' : 'badge-pending';
        rows.push({ member, camp, cl, entries, d5Done, d1Done, d5Pct, d1Pct, overallPct, lastStr, status, statusCls });
      });

      const total = rows.length;
      const complete = rows.filter(r => r.overallPct === 100).length;
      const inProg = rows.filter(r => r.overallPct > 0 && r.overallPct < 100).length;
      ...
      html += `
        <div class="report-section">
          <div class="report-section-header">
            ...
            <div class="report-section-stats">
          ...
          <div class="report-table-wrap">
            <table class="report-table">
              ...
              <tbody>
                ${rows.length === 0
                  ? `<tr><td colspan="7" style="text-align:center;color:var(--text-muted);">No members assigned.</td></tr>`
                  : rows.map(r => `
                  <tr>
                    <td><strong>${escHtml(r.member.name || r.member.username)}</strong><br>
                      <span style="font-size:11px;color:var(--text-muted)">@${escHtml(r.member.username)}</span></td>
                    ...
                    <td><strong style="color:${r.overallPct===100?'#059669':r.overallPct>0?'#D97706':'#DC2626'}">${r.overallPct}%</strong></td>
                    <td><span class="badge ${r.statusCls}">${r.status}</span></td>
                    <td style="font-size:12px;color:var(--text-muted)">${r.lastStr}</td>
                    <td style="font-size:12px;">${(()=>{
                      const lines = r.entries.map(e=>[e.brand,e.platform,e.region].filter(Boolean).join(' · ')).filter(Boolean);
                      if(lines.length===0) return '—';
                      if(lines.length<=2) return lines.join('<br>');
                      const id = `reg-${r.member.uid}-${r.camp.id}`;
                      return `<div><span>${escHtml(lines[0])}</span><span id="${id}-more" style="display:none;">${lines.slice(1).map(l=>'<br>'+escHtml(l)).join('')}</span><br><button onclick="toggleRegList('${id}')" id="${id}-btn" style="font-size:10px;color:var(--blue);background:none;border:none;cursor:pointer;padding:0;margin-top:2px;">+${lines.length-1} more</button></div>`;
                    })()}</td>
                  </tr>`).join('')
                }
              </tbody>
...
// ── Excel export using SheetJS-style manual CSV (no external lib needed) ──
async function exportReportToExcel() {
  const { campList, allChecklists } = window._reportData || {};
  if (!campList) { showToast('Please open the Reports tab first.', 'info'); return; }

  const rows = [['Campaign', 'Member', 'Username', 'D-5 Done', 'D-5 Total', 'D-5 %', 'D-1 Done', 'D-1 Total', 'D-1 %', 'Overall %', 'Status', 'Last Active', 'Brand/Platform/Region']];
  ...
  campList.forEach(camp => {
    (camp.assignedUids || []).forEach(uid => {
      ...
      const d5Pct = Math.round((d5Done / ti) * 100);
      const d1Pct = Math.round((d1Done / ti) * 100);
      const lastStr = cl.lastActive ? new Date(cl.lastActive).toLocaleDateString('en-GB') : '';
      const status = overallPct === 100 ? 'Complete' : overallPct > 0 ? 'In Progress' : 'Not Started';
      const entries = (cl.entries || []).map(e => [e.brand, e.platform, e.region].filter(Boolean).join(' · ')).filter(Boolean).join(' | ');
      rows.push([camp.name, member.name || member.username, member.username, d5Done, ti, d5Pct+'%', d1Done, ti, d1Pct+'%', overallPct+'%', status, lastStr, entries]);
    });
...
function handleImportCsvChange(e) {
  ...
  reader.onload = (ev) => {
    ...
    if (uIdx < 0 || nIdx < 0 || pIdx < 0) {
      showError(document.getElementById('import-members-error'), 'CSV must have columns: username, name, password');
      return;
    }
    document.getElementById('import-members-error').style.display = 'none';
    importedMembersPreview = [];
    rows.slice(1).forEach(r => {
      if (r.length <= Math.max(uIdx, nIdx, pIdx)) return;
      const username = r[uIdx]?.toLowerCase();
      const name = r[nIdx];
      const password = r[pIdx];
      if (username && name && password) importedMembersPreview.push({ username, name, password });
    });
