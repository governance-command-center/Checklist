...
async function renderDataTab() {
  ...
  if (nonAdmins.length === 0) {
    ...
    // Separate team leads and regular members
    ...
    if (teamLeads.length > 0) {
      html += `<div style="font-size:11px;font-weight:600;color:var(--text-muted);text-transform:uppercase;letter-spacing:.05em;padding:6px 0 4px;">Team Leads</div>`;
      html += teamLeads.map(m => {
        const managedCount = (m.managedUids || []).length;
        const managedNames = (m.managedUids || [])
          .slice(0, 3)
          .map(uid => members[uid]?.name || members[uid]?.username || uid)
          .join(', ');
        const moreCount = (m.managedUids || []).length > 3 ? ` +${(m.managedUids || []).length - 3} more` : '';
        return `
        <div class="data-list-row">
          <div style="flex:1;min-width:0;">
            ...
            <div class="data-row-sub">@${escHtml(m.username)} · ${managedCount} member${managedCount !== 1 ? 's' : ''} assigned${managedNames ? ': ' + escHtml(managedNames + moreCount) : ''}</div>
          </div>
          <button class="btn-ghost-light btn-sm" style="color:#2563eb;border-color:#bfdbfe;" onclick="openEditTlModal('${m.uid}')">✏️ Members</button>
          <button class="btn-ghost-light btn-sm" style="font-size:11px;color:#7C3AED;border-color:#C4B5FD;" onclick="openAdminResetPassword('${m.uid}', '${(m.name || m.username).replace(/'/g,"\\'")}')">Reset Pwd</button>
          <button class="btn-ghost-light btn-sm" style="color:#DC2626;border-color:#FCA5A5;" onclick="deleteMember('${m.uid}','${(m.name||m.username).replace(/'/g,"\'")}')">Remove</button>
        </div>`;
      }).join('');
      html += `<div style="height:1px;background:var(--border);margin:8px 0;"></div>`;
      html += `<div style="font-size:11px;font-weight:600;color:var(--text-muted);text-transform:uppercase;letter-spacing:.05em;padding:4px 0 4px;">Members</div>`;
    }
    ...
    if (regularMembers.length === 0) {
      ...
      html += regularMembers.map(m => `
        <div class="data-list-row" id="data-mrow-${m.uid}">
...
async function loadTeamLeadData() {
  ...
  if (managedUids.length === 0) return;
