...
function populateTmplEditorSectionSel() {
  ...
  sel.innerHTML = templateEditorSections.map((s, i) =>
    `<option value="${i}">${escHtml(s.title)}</option>`
  ).join('');
}
...
function renderTmplEditorSection() {
  ...
  document.getElementById('tmpl-section-header').innerHTML = `
    <div style="display:flex;gap:8px;align-items:center;margin-bottom:6px;">
      <input type="text" id="tmpl-section-title-input" value="${escHtml(sec.title)}"
      ...
      <button class="btn-ghost-light" style="color:#DC2626;border-color:#FCA5A5;font-size:12px;" onclick="deleteTmplSection(${idx})">🗑 Delete Section</button>
    </div>`;

  const list = document.getElementById('tmpl-items-list');
  if (sec.items.length === 0) {
    list.innerHTML = '<div style="color:var(--text-muted);font-size:13px;padding:12px 8px;">No items in this section yet. Click "+ Add Item" to start.</div>';
    return;
  }

  // Build grouped item cards with up/down reorder controls
  list.innerHTML = sec.items.map((item, ii) => `
    <div class="editor-item-card" id="eic-${idx}-${ii}">
      <div class="editor-item-card-header">
        <div class="editor-item-num">${ii + 1}</div>
        <div class="editor-item-name-wrap">
          <input type="text" class="editor-item-name" placeholder="Item name…"
            value="${escHtml(item.name)}"
            oninput="templateEditorSections[${idx}].items[${ii}].name=this.value;syncItemCardTitle(${idx},${ii},this.value)" />
        </div>
        <div class="editor-item-reorder">
          <button class="editor-reorder-btn" title="Move up"
            onclick="moveTmplItem(${idx},${ii},-1)" ${ii === 0 ? 'disabled' : ''}>▲</button>
          <button class="editor-reorder-btn" title="Move down"
            onclick="moveTmplItem(${idx},${ii},1)" ${ii === sec.items.length - 1 ? 'disabled' : ''}>▼</button>
        </div>
        <button class="editor-item-del" onclick="deleteTmplItem(${idx},${ii})" title="Remove item">✕</button>
      </div>
      <div class="editor-item-card-body">
        <label class="editor-guide-label">Guide question</label>
        <textarea class="editor-item-guide" placeholder="What should the member check or verify?"
          rows="2" oninput="templateEditorSections[${idx}].items[${ii}].guide=this.value">${escHtml(item.guide)}</textarea>
      </div>
    </div>`).join('');
}

function syncItemCardTitle(secIdx, itemIdx, value) {
  // keep data in sync — already done via oninput; this is a hook for future use
}

function moveTmplItem(secIdx, itemIdx, dir) {
  const sec = templateEditorSections[secIdx];
  if (!sec) return;
  ...
  setTimeout(() => {
    const card = document.getElementById(`eic-${secIdx}-${newIdx}`);
...
function addTmplSection() {
  ...
  templateEditorSections.push({ id: `sec_${Date.now()}`, title: name.trim(), items: [] });
  populateTmplEditorSectionSel();
