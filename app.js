...
async function renderUserDashboard() {
  ...
  try {
    ...
    for (const camp of myCampaigns) {
      ...
      for (const uid of assignedUids) {
        ...
        getEntryBreakdown(cl).forEach(eb => {
          ...
          memberRows.push({ uid, pct: eb.overallPct, label: eb.label });
        });
      }
    ...
    for (const { camp, cTotal, cComplete, cInProgress, cNotStarted, campRate, memberRows } of campCards) {
      ...
      // see one bar per entry of their OWN checklist (labelled when >1 entry).
      const barsHtml = memberRows.map(r => {
        const bColor = r.pct === 100 ? '#059669' : r.pct > 0 ? '#3B82F6' : '#E5E7EB';
        const labelHtml = (!isTl && r.label) ? `<span style="font-size:10px;color:var(--text-muted);min-width:60px;">${escHtml(r.label)}</span>` : '';
