...
async function renderAdminView() {
  ...
  campaignList.forEach(camp => {
    (camp.assignedUids || []).forEach(uid => {
      ...
      // TOTAL_ITEMS rather than lumped together with other entries.
      getEntryBreakdown(cl).forEach(eb => {
        rows.push({
          member, camp,
          entryLabel: eb.label,
...
function renderCompletionChart(rows) {
  ...
  rows.forEach(r => {
    ...
    memberMap[uid].d5Total += TOTAL_ITEMS;
    memberMap[uid].d1Done  += r.d1Done;
    memberMap[uid].d1Total += TOTAL_ITEMS;
  });
...
// { idx, label, d5Done, d1Done, d5Pct, d1Pct, overallPct }. `label` is null
// when there's only the single default entry, so callers can skip showing it.
function getEntryBreakdown(cl) {
  cl = cl || {};
  ...
  return entries.map((entry, idx) => {
    ...
    const d5Pct  = Math.round((d5Done / TOTAL_ITEMS) * 100);
    const d1Pct  = Math.round((d1Done / TOTAL_ITEMS) * 100);
...
function updateUserProgress() {
  ...
  // individually against TOTAL_ITEMS (e.g. "Lazada SG: D-5 45/50, D-1 50/50").
  const breakdown = getEntryBreakdown(campData);
      ...
      <div class="user-prog-entries" style="margin-top:10px;display:flex;flex-direction:column;gap:6px;">
        ${breakdown.map(eb => {
          ...
          return `<div style="display:flex;align-items:center;justify-content:space-between;gap:10px;padding:6px 10px;background:rgba(255,255,255,0.5);border-radius:6px;">
            ...
            <span style="font-size:11px;opacity:0.85;">D-5: ${eb.d5Done}/${TOTAL_ITEMS} (${eb.d5Pct}%) &nbsp;·&nbsp; D-1: ${eb.d1Done}/${TOTAL_ITEMS} (${eb.d1Pct}%)</span>
  ...
  try {
    ...
    Object.values(campaigns).forEach(camp => {
      (camp.assignedUids || []).forEach(uid => {
        ...
        getEntryBreakdown(cl).forEach(eb => {
          rows.push({ member, camp, entryLabel: eb.label, d5Done: eb.d5Done, d1Done: eb.d1Done, cl });
        });
      });
    });
  ...
  try {
    ...
    for (const camp of myCampaigns) {
      ...
      for (const uid of assignedUids) {
        ...
        // completion is computed individually against TOTAL_ITEMS rather
        // than lumped together with other entries (see getEntryBreakdown).
        getEntryBreakdown(cl).forEach(eb => {
          cTotal++;
  ...
  try {
    ...
    campList.forEach(camp => {
      ...
      (camp.assignedUids || []).forEach(uid => {
        ...
        // scale with entry count — never just TOTAL_ITEMS (see getEntryBreakdown).
        const entryCount = entries.length || 1;
        const ti          = TOTAL_ITEMS * entryCount;
    ...
    campList.forEach(camp => {
      (camp.assignedUids || []).forEach(uid => {
        ...
        const overallPct = Math.round(((d5Done + d1Done) / (TOTAL_ITEMS * 2 * entryCount)) * 100);
...
async function renderTeamLeadView() {
  ...
  campsToShow.forEach(camp => {
    (camp.assignedUids || []).filter(u => managedUids.includes(u)).forEach(uid => {
      ...
      const breakdown = getEntryBreakdown(cl);
      const d5Sum = breakdown.reduce((s, e) => s + e.d5Done, 0);
      const d1Sum = breakdown.reduce((s, e) => s + e.d1Done, 0);
      const avg = Math.round(((d5Sum + d1Sum) / (TOTAL_ITEMS * 2 * breakdown.length)) * 100);
  ...
  campsToShow.forEach(camp => {
    ...
    campMembers.forEach(uid => {
      ...
      const d5MemberPct = Math.round((d5Sum / (TOTAL_ITEMS * breakdown.length)) * 100);
      const d1MemberPct = Math.round((d1Sum / (TOTAL_ITEMS * breakdown.length)) * 100);
      const overallPct  = Math.round(((d5Sum + d1Sum) / (TOTAL_ITEMS * 2 * breakdown.length)) * 100);
      ...
      const memberSummaryHtml = breakdown.length > 1 ? `
        <div style="display:flex;gap:16px;font-size:11px;color:var(--text-muted);margin-bottom:10px;padding-bottom:8px;border-bottom:1px dashed var(--border);">
          <span>Member total — D-5: ${d5Sum}/${TOTAL_ITEMS * breakdown.length} (${d5MemberPct}%)</span>
          <span>D-1: ${d1Sum}/${TOTAL_ITEMS * breakdown.length} (${d1MemberPct}%)</span>
        </div>` : '';

      // One D-5/D-1 grid per entry — each entry's completion is computed
      // individually against TOTAL_ITEMS (see getEntryBreakdown).
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">
          <div>
            <div style="display:flex;justify-content:space-between;font-size:11px;font-weight:600;margin-bottom:5px;">
              ...
              <span style="color:var(--navy);">${eb.d5Done}/${TOTAL_ITEMS} &nbsp;${eb.d5Pct}%</span>
            </div>
          ...
          <div>
            <div style="display:flex;justify-content:space-between;font-size:11px;font-weight:600;margin-bottom:5px;">
              ...
              <span style="color:var(--navy);">${eb.d1Done}/${TOTAL_ITEMS} &nbsp;${eb.d1Pct}%</span>
            </div>
            <div style="height:7px;background:var(--border);border-radius:4px;overflow:hidden;">
...
async function renderTlOwnChecklistSection() {
  try {
    ...
    const rows = campList.map(camp => {
      ...
      const d5OwnPct   = Math.round((d5Sum / (TOTAL_ITEMS * breakdown.length)) * 100);
      const d1OwnPct   = Math.round((d1Sum / (TOTAL_ITEMS * breakdown.length)) * 100);
      const overallPct = Math.round(((d5Sum + d1Sum) / (TOTAL_ITEMS * 2 * breakdown.length)) * 100);
      const last = cl.lastActive
        ? new Date(cl.lastActive).toLocaleDateString('en-GB', { day:'numeric', month:'short', year:'2-digit' })
        : 'No activity';
      ...
      const ownSummaryHtml = breakdown.length > 1 ? `
        <div style="display:flex;gap:16px;font-size:11px;color:var(--text-muted);margin-bottom:10px;padding-bottom:8px;border-bottom:1px dashed var(--border);">
          <span>Total — D-5: ${d5Sum}/${TOTAL_ITEMS * breakdown.length} (${d5OwnPct}%)</span>
          <span>D-1: ${d1Sum}/${TOTAL_ITEMS * breakdown.length} (${d1OwnPct}%)</span>
        </div>` : '';
