...
async function renderAdminView() {
  ...
  campaignList.forEach(camp => {
    (camp.assignedUids || []).forEach(uid => {
      ...
      getEntryBreakdown(cl, totalItemsMap[camp.id]).forEach(eb => {
        rows.push({
          member, camp,
          entryLabel: eb.label,
...
function countDoneForEntryIdx(dataObj, entryIdx) {
  if (!dataObj) return 0;
  let n = 0;
...
function getEntryBreakdown(cl, totalItems) {
  const ti = totalItems || getTotalItems();
  cl = cl || {};
  const entries = checklistEntries(cl);
  return entries.map((entry, idx) => {
    const d5Done = countDoneForEntryIdx(cl.d5 || {}, idx);
    const d1Done = countDoneForEntryIdx(cl.d1 || {}, idx);
...
// Resolves the EFFECTIVE total-item count for each campaign in campaignList,
// honoring per-campaign checklist templates (campaign.checklistTemplateId)
// and the global checklist override doc, WITHOUT mutating the shared
...
// campaigns through them would corrupt that and also still only reflect the
// last campaign processed). Returns { [campaignId]: totalItems }.
async function resolveCampaignTotalItems(campaignList) {
  ...
  try {
    ...
    if (glDoc.exists && glDoc.data().sections) {
      const sum = glDoc.data().sections.reduce((s, sec) => s + ((sec.items || []).length), 0);
  ...
  (campaignList || []).forEach(camp => {
    ...
    if (camp.checklistTemplateId) {
      ...
      map[camp.id] = sum > 0 ? sum : globalTotal;
    } else {
      map[camp.id] = globalTotal;
    }
  });
  return map;
}
...
function updateUserProgress() {
  ...
  const breakdown = getEntryBreakdown(campData, ti);
      ...
      html += teamLeads.map(tl => {
        ...
        managedUids.forEach(uid => {
          ...
          Object.values(campaigns).forEach(camp => {
            ...
            const cl = (allChecklists[uid] || {})[camp.id] || {};
            getEntryBreakdown(cl, dataTabTotalItemsMap[camp.id]).forEach(eb => {
  ...
  try {
    ...
    Object.values(campaigns).forEach(camp => {
      (camp.assignedUids || []).forEach(uid => {
        ...
        getEntryBreakdown(cl, clTotalItemsMap[camp.id]).forEach(eb => {
  ...
  try {
    ...
    for (const camp of myCampaigns) {
      ...
      for (const uid of assignedUids) {
        ...
        getEntryBreakdown(cl, dashTotalItemsMap[camp.id]).forEach(eb => {
          cTotal++;
  ...
  try {
    ...
    campList.forEach(camp => {
      ...
      (camp.assignedUids || []).forEach(uid => {
        ...
        const campTotal   = reportTotalItemsMap[camp.id] || TOTAL_ITEMS;
        const ti          = campTotal * entryCount;
    ...
    campList.forEach(camp => {
      (camp.assignedUids || []).forEach(uid => {
        ...
        const campTotal = reportTotalItemsMap[camp.id] || TOTAL_ITEMS;
        const overallPct = Math.round(((d5Done + d1Done) / (campTotal * 2 * entryCount)) * 100);
...
async function exportReportToExcel() {
  ...
  campList.forEach(camp => {
    ...
    (camp.assignedUids || []).forEach(uid => {
      ...
      // getEntryBreakdown / resolveCampaignTotalItems).
      const entryCount = (cl.entries || []).length || 1;
      const campTotal  = (totalItemsMap && totalItemsMap[camp.id]) || TOTAL_ITEMS;
      const ti         = campTotal * entryCount;
      const overallPct = Math.round(((d5Done + d1Done) / (ti * 2)) * 100);
      const d5Pct      = Math.round((d5Done / ti) * 100);
      const d1Pct      = Math.round((d1Done / ti) * 100);
...
async function renderTeamLeadView() {
  ...
  campsToShow.forEach(camp => {
    (camp.assignedUids || []).filter(u => managedUids.includes(u)).forEach(uid => {
      ...
      const breakdown = getEntryBreakdown(cl, tlTotalItemsMap[camp.id]);
      const d5Sum = breakdown.reduce((s, e) => s + e.d5Done, 0);
      const d1Sum = breakdown.reduce((s, e) => s + e.d1Done, 0);
      const ti = tlTotalItemsMap[camp.id] || TOTAL_ITEMS;
      const avg = Math.round(((d5Sum + d1Sum) / (ti * 2 * breakdown.length)) * 100);
  ...
  campsToShow.forEach(camp => {
    ...
    campMembers.forEach(uid => {
      ...
      const cl = (checklistData[uid] || {})[camp.id] || {};
      const ti = tlTotalItemsMap[camp.id] || TOTAL_ITEMS;
      const breakdown = getEntryBreakdown(cl, ti);
      const d5Sum = breakdown.reduce((s, e) => s + e.d5Done, 0);
      const d1Sum = breakdown.reduce((s, e) => s + e.d1Done, 0);
...
async function renderTlOwnChecklistSection() {
  try {
    ...
    const rows = campList.map(camp => {
      ...
      const ti = ownTotalItemsMap[camp.id] || TOTAL_ITEMS;
      const breakdown = getEntryBreakdown(cl, ti);
      const d5Sum = breakdown.reduce((s, e) => s + e.d5Done, 0);
      const d1Sum = breakdown.reduce((s, e) => s + e.d1Done, 0);
