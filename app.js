...
function renderUserChecklist() {
  ...
  entries.forEach((e, i) => {
    ...
    html += `<th colspan="6" class="entry-group-header" style="border-left:3px solid rgba(255,255,255,0.3);">
  ...
  // ── THEAD row 2: [☐] Status | Notes for D-5, then [☐] Status | Notes for D-1 ──
  html += `<tr class="sub-head">`;
  entries.forEach((e, i) => {
    const borderL = i === 0 ? 'border-left:3px solid rgba(255,255,255,0.3)' : 'border-left:3px solid rgba(255,255,255,0.3)';
    html += `
    ...
    <th class="sub-head d1-zone sub-head-notes-col">
      <span class="th-inner" style="display:block;padding:6px 8px;text-align:center;font-size:11px;color:#DDD6FE;">Notes</span>
    </th>`;
  });
  html += `</tr></thead>`;
  ...
  CHECKLIST_SECTIONS.forEach(sec => {
    ...
    html += `<tr class="cat-header">
      ...
      ${Array(entryCount * 6 + 1).fill('<td></td>').join('')}
    </tr>`;
    ...
    sec.items.forEach(item => {
      ...
      entries.forEach((_, ei) => {
        ...
        <td class="d1-cb-col" style="">
          <input type="checkbox" class="row-cb-d1" data-item-id="${item.id}" data-cat="${sec.id}" data-entry-idx="${ei}" data-tab="d1" />
        </td>
