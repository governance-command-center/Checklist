...
// for this checklist: d5Done/d5Pct are reported as 0 and overallPct is
// based on D-1 alone (never divided by a phantom D-5 half).
function getEntryBreakdown(cl, totalItems, validIds, hasD5) {
  ...
  return entries.map((entry, idx) => {
    ...
    const overallPct = d5Enabled
      ? (ti > 0 ? Math.round(((d5Done + d1Done) / (ti * 2)) * 100) : 0)
      : d1Pct;
    return {
      idx,
...
function updateUserProgress() {
  ...
  const entries  = getEntries();
  // For team leads, overall completion ("My Checklist") is based on D-1
  // inputs alone — Done and N/A are treated the same. Regular members
  // keep the combined D-5 + D-1 calculation.
  const done     = isTl ? d1Done : (d5Done + d1Done);
  const total    = isTl ? (ti * entries.length) : (ti * (hasD5 ? 2 : 1) * entries.length);
...
function saveChecklist() {
  ...
  saveTimer = setTimeout(async () => {
    ...
    const isTlSave   = currentUser.role === 'team_lead';
    const hasD5Flag  = getHasD5();
    const d5Done     = hasD5Flag ? countDone(campData.d5 || {}, validIds) : 0;
    const d1Done     = countDone(campData.d1 || {}, validIds);
    const entries    = (campData.entries || [{ brand:'',platform:'',region:'' }]);
    // Team leads' own checklist completion is based on D-1 inputs alone.
    const totalItems = isTlSave
      ? getTotalItems() * entries.length
      : getTotalItems() * (hasD5Flag ? 2 : 1) * entries.length;
    const doneCount  = isTlSave ? d1Done : (d5Done + d1Done);
    const isNowComplete = totalItems > 0 && doneCount >= totalItems;
