...
async function renderAdminView() {
  ...
  campaignList.forEach(camp => {
    (camp.assignedUids || []).forEach(uid => {
      ...
      getEntryBreakdown(cl, totalItemsMap[camp.id]?.total, totalItemsMap[camp.id]?.validIds).forEach(eb => {
        rows.push({
          member, camp,
          entryLabel: eb.label,
          d5Done: eb.d5Done, d1Done: eb.d1Done, overallDone: eb.overallPct,
          d5Pct: eb.d5Pct, d1Pct: eb.d1Pct, totalItems: eb.totalItems,
  ...
  tbody.innerHTML = rows.map(r => {
    ...
    return `<tr>
      ...
      <td>${miniBar(r.d5Pct)} ${r.d5Done}/${r.totalItems}</td>
...
function renderCompletionChart(rows) {
  ...
  const members_list = Object.values(memberMap).map(m => {
    const d5Pct = m.d5Total > 0 ? Math.round((m.d5Done / m.d5Total) * 100) : 0;
    const d1Pct = m.d1Total > 0 ? Math.round((m.d1Done / m.d1Total) * 100) : 0;
    return { ...m, d5Pct, d1Pct, avgPct: Math.round((d5Pct + d1Pct) / 2) };
  ...
  el.innerHTML = members_list.map(m => {
    const nudge = m.avgPct === 0 ? `<button class="completion-nudge" onclick="quickNudge('${escHtml(m.name).replace(/'/g,"\\'")}','${m.uid}')">Nudge</button>` : '';
    return `<div class="completion-member-block" style="margin-bottom:14px;">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px;">
        <div class="completion-name" style="width:auto;font-weight:600;">${escHtml(m.name)}</div>
        ${nudge}
      </div>
      <div class="completion-bar-row" style="margin-bottom:3px;">
        ...
        <div class="completion-track"><div class="completion-fill" style="width:${m.d5Pct}%;background:${barColor(m.d5Pct)};"></div></div>
        <div class="completion-pct">${m.d5Pct}%</div>
      </div>
      <div class="completion-bar-row">
        <div class="completion-name" style="width:32px;font-size:10px;color:var(--text-muted);">D-1</div>
        <div class="completion-track"><div class="completion-fill" style="width:${m.d1Pct}%;background:${barColor(m.d1Pct)};"></div></div>
        <div class="completion-pct">${m.d1Pct}%</div>
      </div>
    </div>`;
  }).join('');
}
...
function getEntryBreakdown(cl, totalItems, validIds) {
  const ti  = totalItems || getTotalItems();
  const ids = validIds || getCurrentValidItemIds();
  cl = cl || {};
  const entries = checklistEntries(cl);
  return entries.map((entry, idx) => {
    const d5Done = countDoneForEntryIdx(cl.d5 || {}, idx, ids);
    const d1Done = countDoneForEntryIdx(cl.d1 || {}, idx, ids);
    const d5Pct  = ti > 0 ? Math.round((d5Done / ti) * 100) : 0;
    const d1Pct  = ti > 0 ? Math.round((d1Done / ti) * 100) : 0;
    const overallPct = ti > 0 ? Math.round(((d5Done + d1Done) / (ti * 2)) * 100) : 0;
    return {
      idx,
      label: entries.length > 1 ? buildEntryLabel(entry, idx) : null,
      d5Done, d1Done, d5Pct, d1Pct, overallPct, totalItems: ti,
    };
  });
}

// Resolves the EFFECTIVE total-item count AND valid item-id set for each
// campaign in campaignList, honoring per-campaign checklist templates
// (campaign.checklistTemplateId) and the global checklist override doc,
// WITHOUT mutating the shared CHECKLIST_SECTIONS / window._TOTAL_ITEMS_OVERRIDE
// state (those represent whichever single checklist table is currently open
// on screen — looping campaigns through them would corrupt that and also
// still only reflect the last campaign processed).
// Returns { [campaignId]: { total, validIds } }.
async function resolveCampaignTotalItems(campaignList) {
  ...
  (campaignList || []).forEach(camp => {
    ...
    if (camp.checklistTemplateId) {
      ...
      if (sum > 0) {
        map[camp.id] = { total: sum, validIds: templateItemIds(tmpl.sections) };
        return;
      }
    }
    map[camp.id] = { total: globalTotal, validIds: globalValidIds };
  });
  return map;
}
...
function renderMemberList(filterQuery) {
  ...
  window._membersSorted = nonAdmins; // cache for filtering
  ...
  if (visible.length === 0) {
    list.innerHTML = q
      ? '<div style="color:var(--text-muted);font-size:13px;padding:8px 10px">No members match your search.</div>'
      : '<div style="color:var(--text-muted);font-size:13px;padding:8px 10px">No members yet.</div>';
    return;
  }
  list.innerHTML = visible.map(m => `
    <div class="member-row" id="mrow-${m.uid}">
      <div>
        ...
        ${m.role === 'team_lead' ? '<span style="font-size:10px;background:#eff6ff;color:#2563eb;border-radius:4px;padding:1px 6px;margin-left:4px;">Team Lead</span>' : ''}
      </div>
...
function updateUserProgress() {
  ...
  const campData = userChecklist[selectedCampaignId] || {};
  const done     = countDone(campData.d5 || {}, ids) + countDone(campData.d1 || {}, ids);
  const entries  = getEntries();
  const total    = ti * 2 * entries.length;
  const pct      = total > 0 ? Math.round((done / total) * 100) : 0;
  const d5Done   = countDone(campData.d5 || {}, ids);
  const d1Done   = countDone(campData.d1 || {}, ids);
  const d5Pct    = total > 0 ? Math.round((d5Done / (ti * entries.length)) * 100) : 0;
  const d1Pct    = total > 0 ? Math.round((d1Done / (ti * entries.length)) * 100) : 0;

  // Per-entry breakdown — each entry's D-5 / D-1 completion is computed
  // individually against this campaign's actual template size (e.g.
  // "Lazada SG: D-5 45/50, D-1 50/50").
  const breakdown = getEntryBreakdown(campData, ti, ids);
  ...
  if (banner) {
    ...
    const entryRowsHtml = breakdown.length > 1 ? `
      <div class="user-prog-entries" style="margin-top:10px;display:flex;flex-direction:column;gap:6px;">
        ${breakdown.map(eb => {
          const eColor = eb.overallPct === 100 ? '#059669' : eb.overallPct >= 50 ? '#D97706' : '#2563EB';
          return `<div style="display:flex;align-items:center;justify-content:space-between;gap:10px;padding:6px 10px;background:rgba(255,255,255,0.5);border-radius:6px;">
            <span style="font-size:12px;font-weight:600;">${escHtml(eb.label || `Entry ${eb.idx + 1}`)}</span>
            <span style="font-size:11px;opacity:0.85;">D-5: ${eb.d5Done}/${eb.totalItems} (${eb.d5Pct}%) &nbsp;·&nbsp; D-1: ${eb.d1Done}/${eb.totalItems} (${eb.d1Pct}%)</span>
            <span style="font-size:12px;font-weight:700;color:${eColor};">${eb.overallPct}%</span>
          </div>`;
        }).join('')}
      </div>` : '';
    ...
    banner.innerHTML = `
      <div class="user-prog-banner">
        ...
        <div class="user-prog-detail">
          <span>D-5: ${d5Done}/${ti * entries.length} (${d5Pct}%)</span>
  ...
  if (nonAdmins.length === 0) {
    ...
    if (teamLeads.length > 0) {
      ...
      html += teamLeads.map(tl => {
        ...
        // Build per-member compliance mini-rows
        let complianceRows = '';
        ...
        managedUids.forEach(uid => {
          ...
          Object.values(campaigns).forEach(camp => {
            ...
            const cl = (allChecklists[uid] || {})[camp.id] || {};
            getEntryBreakdown(cl, dataTabTotalItemsMap[camp.id]?.total, dataTabTotalItemsMap[camp.id]?.validIds).forEach(eb => {
          ...
          complianceRows += `
            <div style="display:flex;align-items:center;gap:8px;padding:4px 0;border-bottom:1px solid var(--border);">
              <div style="flex:1;font-size:12px;font-weight:500;color:var(--text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${escHtml(m.name || m.username)}</div>
        ...
        const tlRate = managedCount > 0 ? Math.round((tlComplete / managedCount) * 100) : 0;
        ...
        <div class="data-list-row data-tl-row" data-name="${escHtml((tl.name || tl.username).toLowerCase())}" data-username="${escHtml(tl.username.toLowerCase())}" style="flex-direction:column;align-items:stretch;gap:0;padding:0;">
          <div style="display:flex;align-items:center;gap:8px;padding:10px 12px;">
            <div style="flex:1;min-width:0;">
              ...
              <div class="data-row-sub">@${escHtml(tl.username)} · ${managedCount} member${managedCount !== 1 ? 's' : ''} · Compliance: <span style="color:${tlRateColor};font-weight:600;">${tlRate}%</span></div>
            </div>
            ...
            ${managedCount > 0 ? `<button class="btn-ghost-light btn-sm" style="font-size:11px;" onclick="toggleTlDash('${dashId}')">📊 View</button>` : ''}
          </div>
          ${managedCount > 0 ? `
  ...
  try {
    ...
    Object.values(campaigns).forEach(camp => {
      (camp.assignedUids || []).forEach(uid => {
        ...
        getEntryBreakdown(cl, clTotalItemsMap[camp.id]?.total, clTotalItemsMap[camp.id]?.validIds).forEach(eb => {
          rows.push({ member, camp, entryLabel: eb.label, d5Done: eb.d5Done, d1Done: eb.d1Done, d5Pct: eb.d5Pct, d1Pct: eb.d1Pct, totalItems: eb.totalItems, cl });
        });
      });
    });
...
function updateUserSidebarProgress() {
  ...
  const done     = countDone(campData.d5 || {}, ids) + countDone(campData.d1 || {}, ids);
  const entries  = getEntries();
  const total    = getTotalItems() * 2 * entries.length;
  const pct      = total > 0 ? Math.round((done / total) * 100) : 0;
  ...
  try {
    ...
    for (const camp of myCampaigns) {
      ...
      for (const uid of assignedUids) {
        ...
        getEntryBreakdown(cl, dashTotalItemsMap[camp.id]?.total, dashTotalItemsMap[camp.id]?.validIds).forEach(eb => {
          cTotal++;
          if (eb.overallPct === 100)  cComplete++;
          else if (eb.overallPct > 0) cInProgress++;
          else                        cNotStarted++;
          memberRows.push({ uid, d5Pct: eb.d5Pct, d1Pct: eb.d1Pct, label: eb.label });
        });
      }
...
async function loadChecklistOverrides(campaignId) {
  try {
    const campSrc = (currentUser?.role === 'team_lead') ? tlOwnCampaigns : campaigns;
    // If a campaignId is provided and it has a template assigned, load that template
    if (campaignId && campSrc[campaignId]?.checklistTemplateId) {
      const tmplId = campSrc[campaignId].checklistTemplateId;
      const tmplDoc = await db.collection('settings').doc('checklistTemplates').get();
      if (tmplDoc.exists) {
  ...
  try {
    ...
    campList.forEach(camp => {
      ...
      (camp.assignedUids || []).forEach(uid => {
        ...
        const d5Done  = countDone(cl.d5 || {}, campInfo.validIds);
                ...
                ${rows.length === 0
                  ...
                  : rows.map(r => {
                    ...
                    return `<tr style="${isAtRisk ? 'background:rgba(220,38,38,0.03);' : ''}">
                      ...
                      <td>${miniBar(r.d5Pct)} ${r.d5Done}/${r.ti}</td>
    ...
    campList.forEach(camp => {
      (camp.assignedUids || []).forEach(uid => {
        ...
        const overallPct = Math.round(((d5Done + d1Done) / (campInfo.total * 2 * entryCount)) * 100);
...
async function exportReportToExcel() {
  ...
  campList.forEach(camp => {
    ...
    (camp.assignedUids || []).forEach(uid => {
      ...
      if (!member) return;
      const cl         = (allChecklists[uid] || {})[camp.id] || {};
      const campInfo   = (totalItemsMap && totalItemsMap[camp.id]) || { total: TOTAL_ITEMS, validIds: null };
      // Filter by validIds so leftover "done" flags from an item that was
      // since removed from the template don't inflate the count past 100%.
      const d5Done     = countDone(cl.d5 || {}, campInfo.validIds);
      const d1Done     = countDone(cl.d1 || {}, campInfo.validIds);
...
async function renderTeamLeadView() {
  ...
  // ── Your own checklist progress, so the team lead's work is monitored
  //    right alongside the team they manage ──
  const ownHtml = await renderTlOwnChecklistSection();

  if (managedUids.length === 0) {
    el.innerHTML = ownHtml + '<div style="padding:3rem;text-align:center;color:var(--text-muted);">No members assigned to you yet. Contact the admin.</div>';
    return;
  }
  ...
  if (campsToShow.length === 0) {
    el.innerHTML = ownHtml + '<div style="padding:3rem;text-align:center;color:var(--text-muted);">No campaigns found for your team.</div>';
    return;
  }

  // Load checklists for all managed members
  const checklistData = {};
  ...
  <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:24px;">
    <div style="background:var(--surface);border:1px solid var(--border);border-radius:10px;padding:14px 18px;text-align:center;">
      ...
      <div style="font-size:11px;color:var(--text-muted);margin-top:2px;">Total Members</div>
    </div>
    <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:10px;padding:14px 18px;text-align:center;">
      ...
      <div style="font-size:11px;color:#16a34a;margin-top:2px;">✓ Completed</div>
    </div>
    <div style="background:#fffbeb;border:1px solid #fde68a;border-radius:10px;padding:14px 18px;text-align:center;">
      ...
      <div style="font-size:11px;color:#d97706;margin-top:2px;">— Pending</div>
    </div>
  ...
  campsToShow.forEach(camp => {
    ...
    campMembers.forEach(uid => {
      ...
      const info = tlTotalItemsMap[camp.id] || { total: TOTAL_ITEMS, validIds: null };
      const ti = info.total;
      const breakdown = getEntryBreakdown(cl, ti, info.validIds);
      const d5Sum = breakdown.reduce((s, e) => s + e.d5Done, 0);
      const d1Sum = breakdown.reduce((s, e) => s + e.d1Done, 0);
      ...
      const overallPct  = Math.round(((d5Sum + d1Sum) / (ti * 2 * breakdown.length)) * 100);
      ...
      const memberSummaryHtml = breakdown.length > 1 ? `
        <div style="display:flex;gap:16px;font-size:11px;color:var(--text-muted);margin-bottom:10px;padding-bottom:8px;border-bottom:1px dashed var(--border);">
          <span>Member total — D-5: ${d5Sum}/${ti * breakdown.length} (${d5MemberPct}%)</span>
  ...
  el.innerHTML = ownHtml + (html || '<div style="padding:3rem;text-align:center;color:var(--text-muted);">No checklist data yet for your team.</div>');
}

// ── Team lead's own checklist progress card (for "monitoring as a team") ──
async function renderTlOwnChecklistSection() {
  try {
    const campsSnap = await db.collection('campaigns')
      .where('assignedUids', 'array-contains', currentUser.uid)
      .get();
    const ownCamps = {};
    campsSnap.forEach(doc => { ownCamps[doc.id] = { ...doc.data(), id: doc.id }; });
    tlOwnCampaigns = ownCamps; // keep the Checklist tab's picker in sync

    const campList = Object.values(ownCamps);
    if (campList.length === 0) return '';

    const checkSnap = await db.collection('checklists').doc(currentUser.uid).get();
    const ownChecklist = checkSnap.exists ? checkSnap.data() : {};

    // Campaigns can use a non-default checklist template — resolve each
    // campaign's real total item count (see resolveCampaignTotalItems).
    const ownTotalItemsMap = await resolveCampaignTotalItems(campList);

    const rows = campList.map(camp => {
      const cl = ownChecklist[camp.id] || {};
      const info = ownTotalItemsMap[camp.id] || { total: TOTAL_ITEMS, validIds: null };
      const ti = info.total;
      const breakdown = getEntryBreakdown(cl, ti, info.validIds);
      const d5Sum = breakdown.reduce((s, e) => s + e.d5Done, 0);
      const d1Sum = breakdown.reduce((s, e) => s + e.d1Done, 0);
      // Aggregate across this campaign's entries — D-5 and D-1 computed
      // separately, never blended into one number.
      const d5OwnPct   = Math.round((d5Sum / (ti * breakdown.length)) * 100);
      const d1OwnPct   = Math.round((d1Sum / (ti * breakdown.length)) * 100);
      const overallPct = Math.round(((d5Sum + d1Sum) / (ti * 2 * breakdown.length)) * 100);
      const last = cl.lastActive
        ? new Date(cl.lastActive).toLocaleDateString('en-GB', { day:'numeric', month:'short', year:'2-digit' })
        : 'No activity';
      ...
      // Summary line across all entries (only needed when there's more than one)
      const ownSummaryHtml = breakdown.length > 1 ? `
        <div style="display:flex;gap:16px;font-size:11px;color:var(--text-muted);margin-bottom:10px;padding-bottom:8px;border-bottom:1px dashed var(--border);">
          <span>Total — D-5: ${d5Sum}/${ti * breakdown.length} (${d5OwnPct}%)</span>
          <span>D-1: ${d1Sum}/${ti * breakdown.length} (${d1OwnPct}%)</span>
        </div>` : '';

      // One D-5/D-1 grid per entry — see getEntryBreakdown for why.
      const entryGridsHtml = breakdown.map(eb => `
        ${eb.label ? `<div style="font-size:11px;font-weight:600;color:var(--navy);margin:${eb.idx > 0 ? '14px' : '0'} 0 6px;">${escHtml(eb.label)}</div>` : ''}
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">
          <div>
            ...
            <div style="height:7px;background:var(--border);border-radius:4px;overflow:hidden;">
              <div style="height:100%;width:${eb.d5Pct}%;background:${eb.d5Pct===100?'#16a34a':eb.d5Pct>0?'#f59e0b':'#e5e7eb'};border-radius:4px;"></div>
            </div>
          </div>
          <div>
            <div style="display:flex;justify-content:space-between;font-size:11px;font-weight:600;margin-bottom:5px;">
            ...
            <div style="height:7px;background:var(--border);border-radius:4px;overflow:hidden;">
              <div style="height:100%;width:${eb.d1Pct}%;background:${eb.d1Pct===100?'#16a34a':eb.d1Pct>0?'#f59e0b':'#e5e7eb'};border-radius:4px;"></div>
            </div>
          </div>
        </div>`).join('');

      return `
      <div style="background:var(--surface);border:1px solid var(--blue);border-radius:10px;padding:16px 18px;">
        <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:12px;flex-wrap:wrap;gap:8px;">
          <div>
            <div style="font-weight:600;font-size:14px;color:var(--navy);">📁 ${escHtml(camp.name)}</div>
            <div style="font-size:11px;color:var(--text-muted);margin-top:2px;">Last active: ${last}</div>
          </div>
          <div style="display:flex;align-items:center;gap:8px;">
            <span style="font-size:11px;font-weight:600;padding:3px 10px;border-radius:20px;background:${badgeBg};color:${badgeCls};border:1px solid ${badgeCls}33;">${badgeLbl}</span>
            <button class="btn-outline" style="font-size:12px;padding:4px 12px;white-space:nowrap;" onclick="switchToTlChecklistTab('${camp.id}')">Open Checklist</button>
          </div>
        </div>
        ${ownSummaryHtml}
        ${entryGridsHtml}
    ...
    return `
    <div style="margin-bottom:28px;">
      <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.07em;color:var(--blue);margin-bottom:10px;padding-bottom:8px;border-bottom:1px solid var(--border);">
        🧑‍💼 Your Checklist — ${escHtml(currentUser.name || currentUser.username)}
      </div>
      <div style="display:flex;flex-direction:column;gap:10px;">${rows}</div>
    </div>`;
  } catch (e) {
    console.warn('Could not load your own checklist progress', e);
    return '';
  }
}
