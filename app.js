...
function applyBulkStatus(status) {
  const panel = document.getElementById('bulk-action-panel');
  const tab   = panel.dataset.tab || 'd5';

  // Collect checked row checkboxes
  const checked = [...document.querySelectorAll('.row-cb:checked')];
  if (checked.length === 0) { closeBulkPanel(); return; }

  checked.forEach(cb => {
    const itemId    = cb.dataset.itemId;
    const entryIdx  = parseInt(cb.dataset.entryIdx);
    const checkTab  = cb.dataset.tab;
    ensurePath(checkTab);
    const key = entryIdx === 0 ? itemId : `${itemId}_e${entryIdx}`;
    userChecklist[selectedCampaignId][checkTab][key] = {
      ...(userChecklist[selectedCampaignId][checkTab][key] || {}), status
    };
    // Update the dropdown in the table
    const sel = document.getElementById(`sel-${checkTab}-${itemId}-${entryIdx}`);
    if (sel) {
      sel.value = status;
...
function renderUserChecklist() {
  ...
  <div id="bulk-action-panel" style="display:none;position:sticky;top:0;z-index:50;
    background:var(--navy);color:white;padding:8px 16px;border-radius:var(--radius);
    margin-bottom:10px;align-items:center;gap:12px;flex-wrap:wrap;box-shadow:0 4px 16px rgba(27,58,107,0.3);">
    <span id="bulk-count-label" style="font-size:13px;font-weight:500;"></span>
    <span style="font-size:12px;opacity:0.7;">Set status to:</span>
    <button class="bulk-btn bulk-done"     onclick="applyBulkStatus('done')">✓ Done</button>
    <button class="bulk-btn bulk-progress" onclick="applyBulkStatus('in-progress')">⟳ In Progress</button>
    <button class="bulk-btn bulk-pending"  onclick="applyBulkStatus('')">— Pending</button>
    <button class="bulk-btn bulk-na"       onclick="applyBulkStatus('na')">N/A</button>
...
function renderCalendarView() {
  const wrap = document.getElementById('user-calendar-view');
  if (!wrap) return;
...
function renderAdminCalendarTab() {
  const wrap = document.getElementById('admin-calendar-wrap');
  if (!wrap) return;

  // Re-use user calendar view but with admin editing powers
  const tempUser = currentUser;
  renderCalendarViewInto(wrap);
}
