...
//
//  Firestore paths:
//    /registrationPolls/{pollId}   — poll meta created by admin
//    /pollResponses/{pollId}/responses/{uid}  — per-member responses
//
...
async function createRegPoll() {
  ...
  try {
    // Close any existing open poll first
    const existing = await db.collection('registrationPolls').where('status','==','open').get();
    const batch = db.batch();
    existing.forEach(d => batch.update(d.ref, { status: 'closed' }));
    await batch.commit();

    const pollRef = await db.collection('registrationPolls').add({
      name, desc,
      status:    'open',
      createdAt: firebase.firestore.FieldValue.serverTimestamp(),
      createdBy: ADMIN_UID,
      readBy:    [],   // track who has seen the "Register Now" broadcast
    });
...
async function loadAndRenderRegPollAdmin() {
  ...
  try {
    // Load most recent open or latest poll
    let snap = await db.collection('registrationPolls').where('status','==','open').orderBy('createdAt','desc').limit(1).get();
    if (snap.empty) {
      snap = await db.collection('registrationPolls').orderBy('createdAt','desc').limit(1).get();
    }
    ...
    if (snap.empty) {
      host.innerHTML = `
        <div style="padding:3rem;text-align:center;">
          ...
          <div style="font-size:18px;font-weight:600;color:var(--navy);margin-bottom:6px;">No registration polls yet</div>
          <div style="color:var(--text-muted);margin-bottom:20px;">Create a poll to start collecting brand registrations from your team.</div>
          <button class="btn-primary" style="width:auto;" onclick="openRegPollModal()">📣 Create Registration Poll</button>
        </div>`;
      return;
    }

    const pollDoc = snap.docs[0];
    currentPollData = { id: pollDoc.id, ...pollDoc.data() };
    activePollId = pollDoc.id;

    // Load responses
    const respSnap = await db.collection('pollResponses').doc(pollDoc.id).collection('responses').get();
    pollResponses = {};
    respSnap.forEach(d => { pollResponses[d.id] = d.data(); });

    renderRegPollAdmin();
  } catch(e) {
    host.innerHTML = '<div style="padding:2rem;color:var(--danger);">Failed to load registration data.</div>';
    console.error(e);
  }
}
    ...
    <div class="reg-poll-admin-wrap">
      ...
      <div class="reg-poll-header-card">
        <div class="reg-poll-header-left">
          <div class="reg-poll-status-pill ${isOpen ? 'rp-open' : 'rp-closed'}">${isOpen ? '🟢 Open' : '🔴 Closed'}</div>
        ...
        <div class="reg-poll-header-right">
          <button class="btn-primary" style="width:auto;" onclick="openRegPollModal()">📣 New Poll</button>
          ${isOpen ? `<button class="btn-outline" style="border-color:#FCA5A5;color:#FCA5A5;" onclick="closeRegPoll()">🔒 Close Poll</button>` : ''}
        </div>
      </div>
      ...
      <div class="reg-poll-section-grid">
          ...
          <div class="rp-member-list">
            ${registered.length === 0 ? '<div class="rp-empty">No registrations yet.</div>' :
              registered.map(({ member, resp }) => `
                <div class="rp-member-card rp-registered">
                  <div class="rp-member-name">${escHtml(member.name || member.username)}</div>
                  <div class="rp-reg-tags">
                    ${(resp.registrations || []).map(r => `
                      <span class="rp-reg-tag">${escHtml(r.platform)} · ${escHtml(r.region)}</span>
                    `).join('')}
                  </div>
        ...
        <div class="reg-poll-col">
          ...
          <div class="reg-poll-col-header" style="color:#DC2626;margin-top:16px;">❓ No Response (${pending.length})</div>
          <div class="rp-member-list">
...
async function closeRegPoll() {
  ...
  await db.collection('registrationPolls').doc(activePollId).update({ status: 'closed' });
...
// Modified createCampaign to handle poll-based creation
const _originalCreateCampaign = window.createCampaign;
window.createCampaign = async function() {
  ...
  try {
    ...
    // Build prefill data from poll responses
    let prefillData = {};
    if (pollId && pollResponses) {
      assignedUids.forEach(uid => {
        const resp = pollResponses[uid];
        if (resp && resp.status === 'registered' && resp.registrations) {
          prefillData[uid] = resp.registrations.map(r => ({
            label: `${r.brand || ''}_${r.platform}_${r.region}`.replace(/^_|_$/g,''),
            brand: r.brand || '',
            platform: r.platform,
            region: r.region,
          }));
    ...
    // If from poll: pre-populate each member's checklist entries
    if (Object.keys(prefillData).length > 0) {
      const batchOps = [];
      for (const [uid, entries] of Object.entries(prefillData)) {
        batchOps.push(
    ...
    // Send campaign-ready broadcast to assigned members
    await db.collection('broadcasts').add({
      type:        'custom',
      message:     `🚀 Campaign "${name}" is ready! Your checklist has been pre-filled with your registration details. Head to the Checklist tab to get started.`,
      targetUid:   null,
      targetName:  'everyone',
      campaignId:  ref.id,
      campaignName: name,
...
async function checkForPendingPoll() {
  ...
  try {
    const snap = await db.collection('registrationPolls').where('status','==','open').orderBy('createdAt','desc').limit(1).get();
    ...
    const respDoc = await db.collection('pollResponses').doc(pollId).collection('responses').doc(currentUser.uid).get();
    if (respDoc.exists) return; // already responded

    userPendingPollId = pollId;
    currentPollData   = { id: pollId, ...poll.data() };
...
function buildUserRegRows() {
  return userRegEntries.map((entry, i) => `
    <div class="user-reg-entry" id="ureg-entry-${i}">
      <div class="user-reg-entry-num">${i + 1}</div>
      <div class="user-reg-entry-fields">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
          <div class="field" style="margin-bottom:0;">
            ...
            <select onchange="userRegEntries[${i}].platform = this.value" style="font-size:13px;">
          ...
          <div class="field" style="margin-bottom:0;">
            ...
            <select onchange="userRegEntries[${i}].region = this.value" style="font-size:13px;">
              <option value="">Select…</option>
      ...
      ${i > 0 ? `<button onclick="removeUserRegEntry(${i})" style="background:rgba(239,68,68,0.15);border:1px solid rgba(239,68,68,0.3);color:#FCA5A5;border-radius:50%;width:24px;height:24px;font-size:12px;cursor:pointer;flex-shrink:0;align-self:flex-start;margin-top:4px;">✕</button>` : '<div style="width:24px;"></div>'}
    </div>
  `).join('');
}
...
async function submitUserRegistration() {
  ...
  try {
    await db.collection('pollResponses').doc(userPendingPollId)
      .collection('responses').doc(currentUser.uid).set({
        status:        'registered',
...
// ─── checkForPendingPoll is called from loadMemberData directly ───
// (patched into original via call at bottom of loadMemberData)

// ─── Patch switchAdminTab to add 'registration' tab ─────────
...
window.switchAdminTab = function(tab) {
  ...
  if (tab === 'registration') {
    // Hide all other tabs
    ['progress','calendar','data'].forEach(t => {
      const el = document.getElementById('admin-tab-' + t);
      if (el) el.style.display = 'none';
    });
    regTab.style.display = 'block';
    document.querySelectorAll('.admin-tab-btn').forEach(b => b.classList.remove('active'));
    const btn = document.getElementById('atab-registration');
    if (btn) btn.classList.add('active');
