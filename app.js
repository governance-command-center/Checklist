...
function renderCompletionChart(rows) {
  ...
  Object.values(memberMap).forEach(m => {
    ...
    m.avgPct = Math.round((m.d5Pct + m.d1Pct) / 2);
  });

  // Group members under their team lead (managedUids) — combined progress
  // is computed from the raw done/total sums of every managed member, not
  // an average of percentages, so it stays accurate regardless of how many
  // items each member's campaign(s) have.
  ...
  const leadGroups = teamLeads.map(tl => {
    ...
    const d5Done = memberList.reduce((s, m) => s + m.d5Done, 0);
    const d5Total = memberList.reduce((s, m) => s + m.d5Total, 0);
    const d1Done = memberList.reduce((s, m) => s + m.d1Done, 0);
    const d1Total = memberList.reduce((s, m) => s + m.d1Total, 0);
    ...
    return {
      ...
      d5Pct, d1Pct, avgPct: Math.round((d5Pct + d1Pct) / 2),
      memberCount: memberList.length,
      memberList: memberList.slice().sort((a, b) => b.avgPct - a.avgPct),
    };
  });

  // Members not under any team lead are grouped as "Unassigned" so their
  ...
  el.innerHTML = leadGroups.map((g, idx) => {
    ...
    const memberRowsHtml = g.memberList.map(m => `
      <div style="display:flex;align-items:center;gap:8px;padding:5px 0;border-bottom:1px solid var(--border);">
        <div style="flex:1.4;font-size:12px;font-weight:500;color:var(--text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${escHtml(m.name)}</div>
    ...
    return `<div class="completion-member-block" style="margin-bottom:14px;border:1px solid var(--border);border-radius:10px;padding:10px 12px;">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;cursor:pointer;" onclick="toggleLeadCompletionPanel('${panelId}', this)" title="Click to view individual members' checklist details">
        <div style="display:flex;align-items:center;gap:6px;min-width:0;">
          ...
          <span style="font-size:11px;color:var(--text-muted);white-space:nowrap;">(${g.memberCount} member${g.memberCount !== 1 ? 's' : ''})</span>
        </div>
      </div>
  ...
  try {
    if (camp?.checklistTemplateId) {
      ...
      if (tmplDoc.exists) {
        ...
        const tmpl = templates.find(t => t.id === camp.checklistTemplateId);
        if (tmpl && tmpl.sections && tmpl.sections.length > 0) return tmpl.sections;
      }
    }
    const glDoc = await db.collection('settings').doc('checklist').get();
    if (glDoc.exists && glDoc.data().sections && glDoc.data().sections.length > 0) return glDoc.data().sections;
  } catch (e) { /* fall back to default below */ }
  return DEFAULT_CHECKLIST_SECTIONS;
}
...
async function openReviewModal(uid, campId) {
  ...
  const sections = await resolveCampaignSections(camp);
