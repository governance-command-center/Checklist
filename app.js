  ...
  list.innerHTML = memberList.length === 0
    ...
    : memberList.map(m => `
        <label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:13px;padding:3px 0;">
          <input type="checkbox" value="${m.uid}" style="accent-color:var(--blue);width:15px;height:15px;" />
...
async function addTeamLead() {
  ...
  const managedUids = [...document.querySelectorAll('#tl-member-assign-list input[type=checkbox]:checked')]
    .map(cb => cb.value);
...
// ─────────────────────────────────────────────────────────────
//  CHANGE PASSWORD  (Member / Team Lead self-service)
// ─────────────────────────────────────────────────────────────
