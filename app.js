...
function applyBulkStatus(status, targetTab) {
  ...
  // Collect checked row checkboxes (unique item+entry combos)
  const checked = [...document.querySelectorAll('.row-cb:checked')];
  if (checked.length === 0) { closeBulkPanel(); return; }

  // Get unique item+entry pairs from checked boxes
  const pairs = new Set();
  checked.forEach(cb => {
    pairs.add(`${cb.dataset.itemId}|||${cb.dataset.entryIdx}`);
  });

  pairs.forEach(pair => {
    const [itemId, entryIdxStr] = pair.split('|||');
    const entryIdx = parseInt(entryIdxStr);
    ensurePath(tab);
    const key = entryIdx === 0 ? itemId : `${itemId}_e${entryIdx}`;
    userChecklist[selectedCampaignId][tab][key] = {
      ...(userChecklist[selectedCampaignId][tab][key] || {}), status
    };
    // Update the dropdown in the table
    const sel = document.getElementById(`sel-${tab}-${itemId}-${entryIdx}`);
    if (sel) {
      sel.value = status;
  ...
  updateUserProgress();
  saveChecklist();
  // Don't close panel — allow applying both D-5 and D-1 without re-selecting
...
function renderUserChecklist() {
  ...
  entries.forEach((e, i) => {
    ...
    html += `<th colspan="4" style="text-align:center;border-left:2px solid rgba(255,255,255,0.18)">
      <span class="th-inner" style="display:flex;align-items:center;gap:8px;justify-content:center;">
        <input type="text" id="entry-label-${i}" placeholder="Brand · Platform · Region"
          value="${escHtml(labelVal)}"
          oninput="updateEntryLabel(${i},this.value)"
          style="font-size:12px;font-weight:600;padding:4px 10px;border:1px solid rgba(255,255,255,0.3);border-radius:6px;background:rgba(255,255,255,0.12);color:white;font-family:var(--font);text-align:center;min-width:180px;max-width:260px;" />
  ...
  // ── THEAD row 2: D-5 / D-1 sub-headers ──
  html += `<tr class="sub-head">`;
  entries.forEach((e, i) => {
    const borderStyle = i === 0 ? 'border-left:2px solid rgba(255,255,255,0.18)' : '';
    html += `
    <th class="sub-head d5-col" style="${borderStyle};background:rgba(37,99,235,0.35);">
      <span class="th-inner" style="padding:5px 10px;display:block;text-align:center;">
        <span style="font-size:11px;font-weight:700;color:#93C5FD;letter-spacing:.05em;">D-5 Status</span>
      </span>
    </th>
    <th class="sub-head d5-col" style="background:rgba(37,99,235,0.2);">
      <span class="th-inner" style="padding:5px 6px;display:block;font-size:10px;color:#93C5FD;text-align:center;">D-5 Notes</span>
    </th>
    <th class="sub-head d1-col" style="background:rgba(124,58,237,0.35);">
      <span class="th-inner" style="padding:5px 10px;display:block;text-align:center;">
        <span style="font-size:11px;font-weight:700;color:#C4B5FD;letter-spacing:.05em;">D-1 Status</span>
      </span>
    </th>
    <th class="sub-head d1-col" style="background:rgba(124,58,237,0.2);">
      <span class="th-inner" style="padding:5px 6px;display:block;font-size:10px;color:#C4B5FD;text-align:center;">D-1 Notes</span>
    </th>`;
  });
  html += `</tr></thead>`;
  ...
  CHECKLIST_SECTIONS.forEach(sec => {
    ...
    html += `<tr class="cat-header">
      <td style="padding:6px 8px;text-align:center;">
        <input type="checkbox" class="cat-cb bulk-cat-cb" data-cat-id="${sec.id}"
          onchange="toggleCatCheckboxes(this,'${sec.id}')" title="Select category" />
      </td>
    ...
    sec.items.forEach(item => {
      ...
      entries.forEach((_, ei) => {
        ...
        <td style="background:rgba(124,58,237,0.04);">
